...
func Walk(t *Tree, ch chan int) {
	...
	Walk(t.Left, ch)
	ch <- t.Value
	fmt.Println("Walk Channel ",ch," Value ",t.Value)
	Walk(t.Right, ch)
}
...
func Compare(t1, t2 *Tree) bool {
	...
	for {
		v1, ok1 := <-c1
		v2, ok2 := <-c2
		fmt.Println("Comparing v1 ",v1," ok1 ",ok1," - v2 ",v2," ok2 ",ok2)
		if !ok1 || !ok2 {
			return ok1 == ok2
		}
		if v1 != v2 {
...
func main() {
	...
	fmt.Println(Compare(t1, New(10, 1)), "Same Contents")
